...
main =
	do
		...
		let initialEstimate = [0, 0, 0]
		let solutionVector = jacobi matrixA initialEstimate
		mapM_ print solutionVector

jacobi :: [[Double]] -> [Double] -> [Double]
jacobi matrixA oldEstimate =
	let 
		tol = 0.0001
		rowNumbers = [0, 1..(length matrixA - 1)]
		equations = zipWith (createEquations) rowNumbers matrixA
		newEst = map (plugIn oldEstimate) equations
		finalEstimate = if (abs(sum newEst - sum oldEstimate) < tol) then 
			newEst
		else 
			jacobi matrixA newEst
		in finalEstimate
	--in newEst

--createEquations :: Int -> [Num a] -> [Num c]
createEquations :: Int -> [Double] -> [Double]
createEquations rowNum row =
	let
		coeffs = take ((length row) - 1) row
		-- Ensures diagonal is excluded from sub
		diag = coeffs !! rowNum
		modifiedCoeffs = (take rowNum coeffs) ++ [0] ++ (drop (rowNum + 1) coeffs)

		equationb = map (/diag) modifiedCoeffs
		equation1 = map (* (-1)) equationb

		-- Package results
	in (equation1 ++ [((row!!(length row - 1)) / diag)])

plugIn :: [Double] -> [Double] -> Double
plugIn estimate row = 
	let
		coeffs = take ((length row) - 1) row
		rh = (tail row)!!0
		lhSum = sum (zipWith (*) coeffs estimate)
	in lhSum + row!!((length row) - 1)
--plugIn row value = -3 + 5